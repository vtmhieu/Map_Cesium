...
exports.TileFormats = void 0;
const Buffers_1 = require("../base/Buffers");
const TileDataLayouts_1 = require("./TileDataLayouts");
const TileFormatError_1 = require("./TileFormatError");
/**
...
class TileFormats {
     ...
     */
    static isComposite(buffer) {
        const magic = Buffers_1.Buffers.getMagicString(buffer);
        return magic === "cmpt";
    }
    /**
     ...
     * valid CMPT data.
     */
    static readCompositeTileData(buffer) {
        // Basic checks for magic number, length and version
        const magic = Buffers_1.Buffers.getMagicString(buffer);
        if (magic !== "cmpt") {
            throw new TileFormatError_1.TileFormatError(`Expected magic "cmpt", but found "${magic}"`);
        }
        const headerByteLength = 16;
        if (buffer.length < headerByteLength) {
            throw new TileFormatError_1.TileFormatError(`Expected ${headerByteLength} bytes for ${magic} header, ` +
                `but only got ${buffer.length}`);
        }
        const version = buffer.readUInt32LE(4);
        if (version !== 1) {
            throw new TileFormatError_1.TileFormatError(`Expected version "1", but got: "${version}'"`);
        }
        ...
        for (let i = 0; i < tilesLength; ++i) {
            if (buffer.length < byteOffset + 4) {
                throw new TileFormatError_1.TileFormatError(`Could not read byte length for inner tile ${i} ` +
            ...
            if (buffer.length < byteOffset + innerByteLength) {
                throw new TileFormatError_1.TileFormatError(`Could not buffer with length ${innerByteLength} for ` +
                    `inner tile ${i} at offset ${byteOffset} from buffer ${buffer}`);
            }
            const innerBuffer = buffer.subarray(byteOffset, byteOffset + innerByteLength);
    ...
    static readTileData(buffer) {
        ...
        const tileDataLayout = TileDataLayouts_1.TileDataLayouts.create(buffer);
     ...
     * @internal
     */
    static extractTileData(buffer, tileDataLayout) {
        const magic = Buffers_1.Buffers.getMagicString(buffer);
        const version = buffer.readUInt32LE(4);
        ...
        const featureTableJsonBuffer = buffer.subarray(tileDataLayout.featureTableJson.start, tileDataLayout.featureTableJson.end);
        const featureTableBinaryBuffer = buffer.subarray(tileDataLayout.featureTableBinary.start, tileDataLayout.featureTableBinary.end);
        const batchTableJsonBuffer = buffer.subarray(tileDataLayout.batchTableJson.start, tileDataLayout.batchTableJson.end);
        const batchTableBinaryBuffer = buffer.subarray(tileDataLayout.batchTableBinary.start, tileDataLayout.batchTableBinary.end);
        const payloadBuffer = buffer.subarray(tileDataLayout.payload.start, tileDataLayout.payload.end);
        let featureTableJson = Buffers_1.Buffers.getJson(featureTableJsonBuffer);
        const batchTableJson = Buffers_1.Buffers.getJson(batchTableJsonBuffer);
     ...
     */
    static extractGlbBuffers(tileDataBuffer) {
        const glbBuffers = [];
     ...
     */
    static extractGlbBuffersInternal(tileDataBuffer, glbBuffers) {
     ...
     * @returns The `TileData`
     */
    static createDefaultB3dmTileDataFromGlb(glbData) {
        return TileFormats.createB3dmTileDataFromGlb(glbData, undefined, undefined, undefined, undefined);
    }
    /**
     * Creates a Batched 3D Model (B3DM) `TileData` instance from
     * a buffer that is assumed to contain valid binary glTF (GLB)
     ...
     * @returns The `TileData`
     */
    static createB3dmTileDataFromGlb(glbData, featureTableJson, featureTableBinary, batchTableJson, batchTableBinary) {
     ...
     * @returns The `TileData`
     */
    static createDefaultI3dmTileDataFromGlb(glbData) {
        return TileFormats.createI3dmTileDataFromGlb(glbData, undefined, undefined, undefined, undefined);
    }
    /**
     * Creates an Instanced 3D Model (I3DM) `TileData` instance from
     * a buffer that is assumed to contain valid binary glTF (GLB)
     ...
     * @returns The `TileData`
     */
    static createI3dmTileDataFromGlb(glbData, featureTableJson, featureTableBinary, batchTableJson, batchTableBinary) {
        const defaultFeatureTableJson = {
            INSTANCES_LENGTH: 1,
            POSITION: {
                byteOffset: 0,
     ...
     * @returns The `CompositeTileData`
     */
    static createCompositeTileData(tileDatas) {
        const innerBuffers = [];
     ...
     * @returns The buffer
     */
    static createTileDataBuffer(tileData) {
        ...
        const featureTableJsonBuffer = Buffers_1.Buffers.getJsonBufferPadded(featureTable.json, headerByteLength);
        const featureTableBinaryBuffer = Buffers_1.Buffers.getBufferPadded(featureTable.binary);
        const batchTableJsonBuffer = Buffers_1.Buffers.getJsonBufferPadded(batchTable.json);
        const batchTableBinaryBuffer = Buffers_1.Buffers.getBufferPadded(batchTable.binary);
        const payload = Buffers_1.Buffers.getBufferPadded(tileData.payload);
        const byteLength = headerByteLength +
     ...
     * @returns The buffer
     */
    static createCompositeTileDataBuffer(compositeTileData) {
        const buffers = [];
        ...
        for (let i = 0; i < tilesLength; i++) {
            ...
            const innerBufferPadded = Buffers_1.Buffers.getBufferPadded(innerBuffer, byteLength);
...
exports.TileFormats = TileFormats;
