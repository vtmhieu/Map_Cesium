...
const DataError_1 = require("./DataError");
/**
...
class Buffers {
     ...
     */
    static gzip(inputBuffer) {
        const outputBuffer = zlib_1.default.gzipSync(inputBuffer);
        return outputBuffer;
    }
    /**
     ...
     */
    static gunzip(inputBuffer) {
        let outputBuffer;
        if (Buffers.isGzipped(inputBuffer)) {
            try {
                outputBuffer = zlib_1.default.gunzipSync(inputBuffer);
            }
            catch (e) {
                const message = `Could not gunzip buffer: ${e}`;
                throw new DataError_1.DataError(message);
            }
        }
        else {
     ...
     */
    static getMagicBytes(buffer, byteOffset, byteLength) {
     ...
     * @returns The magic header.
     */
    static getMagicString(buffer, byteOffset) {
        const magicLength = 4;
     ...
     * @returns The buffer
     */
    static getJsonBufferPadded(json, byteOffset) {
        if (!(0, defined_1.defined)(json) || Object.keys(json).length === 0) {
     ...
     * @returns The buffer
     */
    static getBufferPadded(buffer, byteOffset) {
        byteOffset = byteOffset ?? 0;
        const boundary = 8;
        const byteLength = buffer.length;
        const remainder = (byteOffset + byteLength) % boundary;
     ...
     * @internal
     */
    static getUnicodeBOMDescription(buffer) {
     ...
     * @internal
     */
    static startsWith(buffer, bytes) {
     ...
     */
    static createBinaryString(buffer) {
