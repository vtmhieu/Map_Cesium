...
class IndexBuilder {
    ...
    addEntry(key, size) {
        ...
        this.currentEntryOffset += BigInt(key.length + IndexBuilder.ZIP_LOCAL_FILE_HEADER_SIZE + size);
    }
    /**
    ...
    createBuffer() {
        ...
        const encodedEntries = this.entries.map((e) => {
            const hashBuffer = crypto_1.default.createHash("md5").update(e.key).digest();
            return {
                key: e.key,
                offset: e.offset,
 ...
 */
IndexBuilder.ZIP_LOCAL_FILE_HEADER_SIZE = 30;
