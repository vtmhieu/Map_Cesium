...
exports.ArchiveFunctions3tz = void 0;
 ...
 * @internal
 */
class ArchiveFunctions3tz {
    static getLastCentralDirectoryEntry(fd, stat) {
        const bytesToRead = 320;
        const buffer = Buffer.alloc(bytesToRead);
        const offset = stat.size - bytesToRead;
        const length = bytesToRead;
        fs_1.default.readSync(fd, buffer, 0, length, offset);
        let start = 0, end = 0;
        for (let i = buffer.length - 4; i > 0; i--) {
            ...
            if (val === ArchiveFunctions3tz.ZIP_START_OF_CENTRAL_DIRECTORY_HEADER_SIG) {
                start = i;
                break;
            }
        }
    ...
    static getFileContents(fd, buffer, expectedFilename) {
        let comp_size = buffer.readUInt32LE(20);
        const filename_size = buffer.readUInt16LE(28);
        const extra_size = buffer.readUInt16LE(30);
        const extrasStartOffset = ArchiveFunctions3tz.ZIP_CENTRAL_DIRECTORY_STATIC_SIZE + filename_size;
        // ZIP64_BUGFIX: If this size is found, then the size is
        // stored in the extra field
        if (comp_size === 0xffffffff) {
            if (extra_size < 28) {
                throw new TilesetError_1.TilesetError("No zip64 extras buffer found");
            }
            ...
            if (extra_tag !== ArchiveFunctions3tz.ZIP64_EXTENDED_INFORMATION_EXTRA_SIG) {
                throw new TilesetError_1.TilesetError("No zip64 extras signature found");
            }
            comp_size = Number(buffer.readBigUInt64LE(extrasStartOffset + 12));
        }
        const filename = buffer.toString("utf8", ArchiveFunctions3tz.ZIP_CENTRAL_DIRECTORY_STATIC_SIZE, ArchiveFunctions3tz.ZIP_CENTRAL_DIRECTORY_STATIC_SIZE + filename_size);
        if (filename !== expectedFilename) {
            throw new TilesetError_1.TilesetError(`Central Directory File Header filename was ${filename}, expected ${expectedFilename}`);
        }
        ...
        */
        ...
        const localFileDataSize = ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE +
            filename_size +
        ...
        fs_1.default.readSync(fd, localFileDataBuffer, 0, localFileDataSize, offset);
        ...
        const dataStartOffset = ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE +
            local_filename_size +
            local_extra_size;
        const fileDataBuffer = localFileDataBuffer.slice(dataStartOffset, dataStartOffset + local_comp_size);
        if (fileDataBuffer.length === 0) {
            throw new TilesetError_1.TilesetError(`Failed to get file data at offset ${dataStartOffset}`);
        }
        return fileDataBuffer;
    }
    static parseIndexData(buffer) {
        if (buffer.length % 24 !== 0) {
            throw new TilesetError_1.TilesetError(`Bad index buffer length: ${buffer.length}`);
        }
    ...
    static md5LessThan(md5hashA, md5hashB) {
    ...
    static zipIndexFind(zipIndex, searchHash) {
        let low = 0;
        ...
        while (low <= high) {
            ...
            else if (ArchiveFunctions3tz.md5LessThan(zipIndex[mid].hash, searchHash)) {
                low = mid + 1;
            }
            else {
                high = mid - 1;
    ...
    static searchIndex(zipIndex, searchPath) {
        const hashedSearchPath = crypto_1.default
            .createHash("md5")
            .update(searchPath)
            .digest();
        //console.log(`Searching index for ${searchPath} (${hashedSearchPath.toString('hex')})`);
        //console.time('Search index');
        const matchedIndex = ArchiveFunctions3tz.zipIndexFind(zipIndex, hashedSearchPath);
    ...
    static parseLocalFileHeader(buffer, expectedFilename) {
        const signature = buffer.readUInt32LE(0);
        if (signature !== 0x04034b50) {
            throw new TilesetError_1.TilesetError(`Bad local file header signature: 0x${signature.toString(16)}`);
        }
        const comp_size = buffer.readUInt32LE(18);
        const filename_size = buffer.readUInt16LE(26);
        const extra_size = buffer.readUInt16LE(28);
        const filename = buffer.toString("utf8", ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE, ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE + filename_size);
        if (filename !== expectedFilename) {
            throw new TilesetError_1.TilesetError(`Local File Header filename was ${filename}, expected ${expectedFilename}`);
        }
        const compressedSize = comp_size;
        if (compressedSize === 0) {
            throw new TilesetError_1.TilesetError("Zip Local File Headers must have non-zero file sizes set.");
        }
        return {
            signature: signature,
            comp_size: comp_size,
    ...
    static readZipLocalFileHeader(fd, offset, path) {
        const headerSize = ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE + path.length;
        const headerBuffer = Buffer.alloc(headerSize);
        //console.log(`readZipLocalFileHeader path: ${path} headerSize: ${headerSize} offset: ${offset}`);
        fs_1.default.readSync(fd, headerBuffer, 0, headerSize, Number(offset));
    ...
    static readZipIndex(fd) {
        const stat = fs_1.default.fstatSync(fd);
        const centralDirectoryEntryData = ArchiveFunctions3tz.getLastCentralDirectoryEntry(fd, stat);
        if (!centralDirectoryEntryData) {
            throw new TilesetError_1.TilesetError("Could not read last central directory entry");
        }
        const indexFileName = "@3dtilesIndex1@";
        const indexFileContents = ArchiveFunctions3tz.getFileContents(fd, centralDirectoryEntryData, indexFileName);
        const zipIndex = ArchiveFunctions3tz.parseIndexData(indexFileContents);
        return zipIndex;
    }
    static readFileName(fd, offset) {
        const headerSize = ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE + 320;
        const headerBuffer = Buffer.alloc(headerSize);
        fs_1.default.readSync(fd, headerBuffer, 0, headerSize, Number(offset));
        const filename_size = headerBuffer.readUInt16LE(26);
        const filename = headerBuffer.toString("utf8", ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE, ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE + filename_size);
        return filename;
    }
    static readEntryData(fd, zipIndex, path) {
        ...
        if (match) {
            const header = ArchiveFunctions3tz.readZipLocalFileHeader(fd, match.offset, path);
            const fileDataOffset = Number(match.offset) +
            ...
            fs_1.default.readSync(fd, fileContentsBuffer, 0, header.comp_size, fileDataOffset);
...
exports.ArchiveFunctions3tz = ArchiveFunctions3tz;
ArchiveFunctions3tz.ZIP_END_OF_CENTRAL_DIRECTORY_HEADER_SIG = 0x06054b50;
ArchiveFunctions3tz.ZIP_START_OF_CENTRAL_DIRECTORY_HEADER_SIG = 0x02014b50;
ArchiveFunctions3tz.ZIP64_EXTENDED_INFORMATION_EXTRA_SIG = 0x0001;
ArchiveFunctions3tz.ZIP_LOCAL_FILE_HEADER_STATIC_SIZE = 30;
ArchiveFunctions3tz.ZIP_CENTRAL_DIRECTORY_STATIC_SIZE = 46;
