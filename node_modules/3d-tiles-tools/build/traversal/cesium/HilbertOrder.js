...
class HilbertOrder {
     ...
     */
    static encode2D(level, x, y) {
        const n = Math.pow(2, level);
        if (level < 1) {
            throw new DeveloperError_1.DeveloperError("Hilbert level cannot be less than 1.");
        }
        if (x < 0 || x >= n || y < 0 || y >= n) {
            throw new DeveloperError_1.DeveloperError("Invalid coordinates for given level.");
        }
        const p = {
            x: x,
            y: y,
     ...
     * @internal
     */
    static decode2D(level, index) {
        if (level < 1) {
            throw new DeveloperError_1.DeveloperError("Hilbert level cannot be less than 1.");
        }
        if (index < BigInt(0) || index >= BigInt(Math.pow(4, level))) {
            throw new DeveloperError_1.DeveloperError("Hilbert index exceeds valid maximum for given level.");
        }
        const n = Math.pow(2, level);
        const p = {
            x: 0,
