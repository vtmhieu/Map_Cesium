...
exports.ImplicitTilings = void 0;
const Quadtrees_1 = require("../spatial/Quadtrees");
const QuadtreeCoordinates_1 = require("../spatial/QuadtreeCoordinates");
const Octrees_1 = require("../spatial/Octrees");
const OctreeCoordinates_1 = require("../spatial/OctreeCoordinates");
const TemplateUris_1 = require("./TemplateUris");
...
class ImplicitTilings {
     ...
     */
    static createSubtreeCoordinatesIterator(implicitTiling) {
        const levels = implicitTiling.subtreeLevels;
        if (levels < 1) {
            throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid number of subtree levels: ${levels}`);
        }
     ...
     * levels is not positive
     */
    static computeNumberOfNodesPerSubtree(implicitTiling) {
        const levels = implicitTiling.subtreeLevels;
        if (levels < 1) {
            throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid number of subtree levels: ${levels}`);
        }
        if (implicitTiling.subdivisionScheme === "QUADTREE") {
            return Quadtrees_1.Quadtrees.computeNumberOfNodesForLevels(levels);
        }
        if (implicitTiling.subdivisionScheme === "OCTREE") {
            return Octrees_1.Octrees.computeNumberOfNodesForLevels(levels);
        }
        throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid subdivisionScheme: ${implicitTiling.subdivisionScheme}`);
    }
    /**
     ...
     */
    static computeNumberOfNodesInLevel(implicitTiling, level) {
        if (level < 0) {
            throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid level: ${level}`);
        }
        const size = 1 << level;
        if (implicitTiling.subdivisionScheme === "QUADTREE") {
            return size * size;
     ...
     */
    static substituteTemplateUri(subdivisionScheme, templateUri, coordinates) {
        if (subdivisionScheme === "QUADTREE") {
            const quadtreeCoordinates = coordinates;
            return TemplateUris_1.TemplateUris.substituteQuadtree(templateUri, quadtreeCoordinates);
        }
        if (subdivisionScheme === "OCTREE") {
            const octreeCoordinates = coordinates;
            return TemplateUris_1.TemplateUris.substituteOctree(templateUri, octreeCoordinates);
        }
     ...
     */
    static createString(coordinates) {
        if (coordinates instanceof QuadtreeCoordinates_1.QuadtreeCoordinates) {
            const quadtreeCoordinates = coordinates;
        ...
        if (coordinates instanceof OctreeCoordinates_1.OctreeCoordinates) {
            const octreeCoordinates = coordinates;
        ...
        throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid coordinates type: ${coordinates}`);
    }
    /**
     ...
     */
    static createRootCoordinates(implicitTiling) {
        const subdivisionScheme = implicitTiling.subdivisionScheme;
        if (subdivisionScheme === "QUADTREE") {
            return new QuadtreeCoordinates_1.QuadtreeCoordinates(0, 0, 0);
        }
        if (subdivisionScheme === "OCTREE") {
            return new OctreeCoordinates_1.OctreeCoordinates(0, 0, 0, 0);
        }
        throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid subdivisionScheme: ${subdivisionScheme}`);
    }
    /**
     ...
     * have a valid `subdivisionScheme`.
     */
    static globalizeCoordinates(implicitTiling, rootCoordinates, coordinates) {
        const subdivisionScheme = implicitTiling.subdivisionScheme;
        if (subdivisionScheme === "QUADTREE") {
            const quadtreeRootCoordinates = rootCoordinates;
            const quadtreeCoordinates = coordinates;
            return ImplicitTilings.globalizeQuadtreeCoords(quadtreeRootCoordinates, quadtreeCoordinates);
        }
        if (subdivisionScheme === "OCTREE") {
            const octreeRootCoordinates = rootCoordinates;
            const octreeCoordinates = coordinates;
            return ImplicitTilings.globalizeOctreeCoords(octreeRootCoordinates, octreeCoordinates);
        }
        throw new ImplicitTilingError_1.ImplicitTilingError(`Invalid subdivisionScheme: ${subdivisionScheme}`);
    }
    /**
     ...
     * @returns The global coordinates
     */
    static globalizeQuadtreeCoords(rootCoords, localCoords) {
        ...
        const globalCoords = new QuadtreeCoordinates_1.QuadtreeCoordinates(globalLevel, globalX, globalY);
        return globalCoords;
    }
    /**
     ...
     * @returns The global coordinates
     */
    static globalizeOctreeCoords(rootCoords, localCoords) {
        const rootLevel = rootCoords.level;
        const rootX = rootCoords.x;
        const rootY = rootCoords.y;
        ...
        const globalCoords = new OctreeCoordinates_1.OctreeCoordinates(globalLevel, globalX, globalY, globalZ);
        return globalCoords;
    }
}
exports.ImplicitTilings = ImplicitTilings;
