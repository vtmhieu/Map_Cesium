...
exports.BinarySubtreeDataResolver = void 0;
const Buffers_1 = require("../base/Buffers");
const BinaryBufferDataResolver_1 = require("../binary/BinaryBufferDataResolver");
const BinaryDataError_1 = require("../binary/BinaryDataError");
 ...
 * @internal
 */
class BinarySubtreeDataResolver {
    /**
     ...
     */
    static async resolveFromJson(subtree, resourceResolver) {
        const binarySubtreeData = await BinarySubtreeDataResolver.resolveInternal(subtree, undefined, resourceResolver);
     ...
     */
    static async resolveFromBuffer(input, resourceResolver) {
        ...
        let subtree;
        try {
            subtreeJson = Buffers_1.Buffers.getJson(jsonBuffer);
            subtree = subtreeJson;
        }
        catch (error) {
            throw new ImplicitTilingError_1.ImplicitTilingError("Could not parse subtree JSON data");
        }
        // Extract the binary buffer
        const binaryStartByteOffset = jsonEndByteOffset;
        const binaryEndByteOffset = binaryStartByteOffset + Number(binaryByteLength);
        const binaryBufferSlice = input.subarray(binaryStartByteOffset, binaryEndByteOffset);
        const binaryBuffer = binaryBufferSlice.length > 0 ? binaryBufferSlice : undefined;
        const binarySubtreeData = await BinarySubtreeDataResolver.resolveInternal(subtree, binaryBuffer, resourceResolver);
        return binarySubtreeData;
    }
    /**
     ...
     */
    static async resolveInternal(subtree, binaryBuffer, resourceResolver) {
        ...
        try {
            binaryBufferData = await BinaryBufferDataResolver_1.BinaryBufferDataResolver.resolve(binaryBufferStructure, binaryBuffer, resourceResolver);
        }
        catch (error) {
            if (error instanceof BinaryDataError_1.BinaryDataError) {
...
exports.BinarySubtreeDataResolver = BinarySubtreeDataResolver;
