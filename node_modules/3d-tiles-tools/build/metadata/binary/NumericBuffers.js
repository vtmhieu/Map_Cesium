...
class NumericBuffers {
     ...
     */
    static getNumericFromBuffer(buffer, index, componentType) {
        const byteSize = MetadataComponentTypes_1.MetadataComponentTypes.byteSizeForComponentType(componentType);
        ...
        throw new MetadataError_1.MetadataError(`Unknown component type: ${componentType}`);
    }
    /**
     ...
     * @throws MetadataError If the given `componentType` is not valid
     */
    static getNumericArrayFromBuffer(buffer, index, arrayLength, componentType) {
        const typedArray = NumericBuffers.getTypedArrayFromBuffer(buffer, index, arrayLength, componentType);
     ...
     * @throws MetadataError If the given `componentType` is not valid
     */
    static getTypedArrayFromBuffer(buffer, index, arrayLength, componentType) {
        const componentSize = MetadataComponentTypes_1.MetadataComponentTypes.byteSizeForComponentType(componentType);
        const elementSize = arrayLength * componentSize;
        const byteOffset = index * elementSize;
        const arrayBuffer = ArrayBuffers_1.ArrayBuffers.fromBuffer(buffer);
        switch (componentType) {
            case "INT8":
     ...
     * @throws MetadataError If the given `componentType` is not valid
     */
    static getNumericBufferAsArray(buffer, componentType) {
        const size = MetadataComponentTypes_1.MetadataComponentTypes.byteSizeForComponentType(componentType);
        const arrayLength = buffer.length / size;
        const typedArray = NumericBuffers.getTypedArrayFromBuffer(buffer, 0, arrayLength, componentType);
        const array = [...typedArray];
        return array;
    }
}
exports.NumericBuffers = NumericBuffers;
