...
exports.BinaryValidationIssues = void 0;
...
class BinaryValidationIssues {
     ...
     */
    static BINARY_INVALID(path, message) {
     ...
     * @returns The `ValidationIssue`
     */
    static BINARY_INVALID_VALUE(path, name, expected, actual) {
    ...
    static _BINARY_INVALID_VALUE(path, message) {
     ...
     * @returns The `ValidationIssue`
     */
    static BINARY_INVALID_LENGTH(path, name, expectedLength, actualLength) {
        const message = `The length of ${name} must be ${expectedLength} ` +
            `but is ${actualLength}`;
        return BinaryValidationIssues._BINARY_INVALID_LENGTH(path, message);
    }
    // Internal method for BINARY_INVALID_LENGTH
    static _BINARY_INVALID_LENGTH(path, message) {
     ...
     * @returns The `ValidationIssue`
     */
    static BINARY_INVALID_ALIGNMENT(path, name, expectedAlignment, actualValue, misalignment) {
    ...
    static _BINARY_INVALID_ALIGNMENT(path, message) {
     ...
     * @returns The `ValidationIssue`
     */
    static BINARY_INVALID_ALIGNMENT_legacy(path, name, expectedAlignment) {
        const message = `The ${name} must be aligned to ${expectedAlignment} bytes`;
        return BinaryValidationIssues._BINARY_INVALID_ALIGNMENT(path, message);
    }
}
exports.BinaryValidationIssues = BinaryValidationIssues;
