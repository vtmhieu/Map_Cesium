...
exports.TemplateUriValidator = void 0;
...
class TemplateUriValidator {
     ...
     */
    static validateTemplateUri(path, name, uri, subdivisionScheme, context) {
        // The uri MUST be a string
        if (!BasicValidator_1.BasicValidator.validateString(path, name, uri, context)) {
            return false;
        }
        let result = true;
        ...
        for (let i = 0; i < expressions.length; i++) {
            ...
            if (!validVariableNames.includes(variableName)) {
                const issue = SemanticValidationIssues_1.SemanticValidationIssues.TEMPLATE_URI_INVALID_VARIABLE_NAME(path, variableName, validVariableNames);
                ...
                expectedVariableNames = expectedVariableNames.filter((e) => e !== variableName);
            }
        }
        // If one of the valid variable names was not used,
        // create a warning
        if (expectedVariableNames.length > 0) {
            const issue = SemanticValidationIssues_1.SemanticValidationIssues.TEMPLATE_URI_MISSING_VARIABLE_NAME(path, expectedVariableNames);
            context.addIssue(issue);
        }
        return result;
    }
}
exports.TemplateUriValidator = TemplateUriValidator;
