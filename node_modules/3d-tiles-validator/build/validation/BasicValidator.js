...
const ValidationIssueUtils_1 = require("../issues/ValidationIssueUtils");
...
class BasicValidator {
     ...
     */
    static validateIdentifierString(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
        const idRegex = /^[a-zA-Z_][a-zA-Z0-9_]*$/;
        if (!idRegex.test(value)) {
            const issue = JsonValidationIssues_1.JsonValidationIssues.STRING_PATTERN_MISMATCH(path, name, value, idRegex.toString());
     ...
     */
    static validateOptionalString(containingPath, containingObject, name, context) {
     ...
     */
    static validateDefined(path, name, value, context) {
        if ((0, _3d_tiles_tools_1.defined)(value)) {
            return true;
        }
        const issue = JsonValidationIssues_1.JsonValidationIssues.PROPERTY_MISSING(path, name);
        context.addIssue(issue);
        return false;
    }
    /**
     ...
     * length
     */
    static validateArray(path, name, value, minLength, maxLength, expectedElementType, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
        if (!Array.isArray(value)) {
            const issue = JsonValidationIssues_1.JsonValidationIssues.TYPE_MISMATCH(path, name, "Array", typeof value);
            context.addIssue(issue);
            return false;
        }
        if (((0, _3d_tiles_tools_1.defined)(minLength) && value.length < minLength) ||
            ((0, _3d_tiles_tools_1.defined)(maxLength) && value.length > maxLength)) {
            const rangeDescription = ValidationIssueUtils_1.ValidationIssueUtils.describeSimpleRange(minLength, maxLength);
            const message = `Array '${name}' must have a length of ${rangeDescription}, ` +
                `but the actual length is ${value.length}`;
            const issue = JsonValidationIssues_1.JsonValidationIssues.ARRAY_LENGTH_MISMATCH(path, message);
            context.addIssue(issue);
            return false;
        }
        if ((0, _3d_tiles_tools_1.defined)(expectedElementType)) {
            for (let index = 0; index < value.length; index++) {
                const element = value[index];
                if (typeof element !== expectedElementType) {
                    const issue = JsonValidationIssues_1.JsonValidationIssues.ARRAY_ELEMENT_TYPE_MISMATCH(path + "/" + index, name, index, expectedElementType, typeof element);
                    context.addIssue(issue);
                    return false;
                }
            }
     ...
     */
    static validateArrayElementsUnique(path, name, array, context) {
        let result = true;
        for (let i = 0; i < array.length; i++) {
            const value = array[i];
            const index = array.indexOf(value);
            if (index != i) {
                const issue = JsonValidationIssues_1.JsonValidationIssues.ARRAY_ELEMENT_NOT_UNIQUE(path, name, value);
                context.addIssue(issue);
                result = false;
            }
        }
     ...
     */
    static validateObject(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
     ...
     * @returns Whether the given value has the expected type
     */
    static validateString(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
     ...
     * @returns Whether the given value has the expected type
     */
    static validateNumber(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
     ...
     * @returns Whether the given value has the expected type
     */
    static validateBoolean(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
     ...
     * @returns Whether the given value has the expected type
     */
    static validateType(path, name, value, expectedType, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
        ...
        const issue = JsonValidationIssues_1.JsonValidationIssues.TYPE_MISMATCH(path, name, expectedType, typeof value);
        context.addIssue(issue);
        return false;
    }
    /**
     ...
     * @returns Whether the given value has the expected type
     */
    static validateInteger(path, name, value, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
        ...
        const issue = JsonValidationIssues_1.JsonValidationIssues.TYPE_MISMATCH(path, name, "integer", typeof value);
        context.addIssue(issue);
        return false;
    }
    /**
     ...
     */
    static validateNumberRange(path, name, value, min, minInclusive, max, maxInclusive, context) {
        if (!BasicValidator.validateNumber(path, name, value, context)) {
            return false;
        }
        return BasicValidator._validateNumberRangeInternal(path, name, value, min, minInclusive, max, maxInclusive, context);
    }
    /**
     * Validate that the given value is in the specified range.
     *
     ...
     * @returns Whether the given value is in the expected range
     */
    static validateIntegerRange(path, name, value, min, minInclusive, max, maxInclusive, context) {
     ...
     * @returns Whether the given value is in the expected range
     */
    static _validateNumberRangeInternal(path, name, value, min, minInclusive, max, maxInclusive, context) {
        if ((0, _3d_tiles_tools_1.defined)(min) && (0, _3d_tiles_tools_1.defined)(max)) {
            ...
            if (!validMin) {
                const minComparison = minInclusive
                    ...
                    : "greater than";
                const message = `The '${name}' property must be ` +
     ...
     * @returns Whether the given value has the expected type
     */
    static validateEnum(path, name, value, allowedValues, context) {
        if (!BasicValidator.validateDefined(path, name, value, context)) {
            return false;
        }
        if (!allowedValues.includes(value)) {
            const message = `The '${name}' property has the value ` +
                `${value}, but must be one of ${allowedValues}`;
            const issue = JsonValidationIssues_1.JsonValidationIssues.VALUE_NOT_IN_LIST(path, message);
            context.addIssue(issue);
            return false;
        }
        return true;
     ...
     * @returns Whether the given value has the expected type
     */
    static validateNumberOfProperties(path, name, value, minProperties, maxProperties, context) {
        if (!BasicValidator.validateObject(path, name, value, context)) {
            return false;
        }
        const numProperties = Object.keys(value).length;
        if (((0, _3d_tiles_tools_1.defined)(minProperties) && numProperties < minProperties) ||
            ((0, _3d_tiles_tools_1.defined)(maxProperties) && numProperties > maxProperties)) {
            const rangeDescription = ValidationIssueUtils_1.ValidationIssueUtils.describeSimpleRange(minProperties, maxProperties);
            const message = `Object '${name}' must have ${rangeDescription} ` +
                `properties, but has ${numProperties} properties`;
            const issue = JsonValidationIssues_1.JsonValidationIssues.NUMBER_OF_PROPERTIES_MISMATCH(path, message);
            context.addIssue(issue);
            return false;
        }
        return true;
     ...
     * @returns Whether the given value has the expected type
     */
    static validateStringLength(path, name, value, minLength, maxLength, context) {
        ...
        if (((0, _3d_tiles_tools_1.defined)(minLength) && value.length < minLength) ||
            ((0, _3d_tiles_tools_1.defined)(maxLength) && value.length > maxLength)) {
            const rangeDescription = ValidationIssueUtils_1.ValidationIssueUtils.describeSimpleRange(minLength, maxLength);
            const message = `String '${name}' must have a length of ${rangeDescription}, ` +
                `but the actual length is ${value.length}`;
            const issue = JsonValidationIssues_1.JsonValidationIssues.ARRAY_LENGTH_MISMATCH(path, message);
            context.addIssue(issue);
            return false;
