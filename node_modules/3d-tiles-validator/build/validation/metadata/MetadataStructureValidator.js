...
exports.MetadataStructureValidator = void 0;
 ...
 * @internal
 */
class MetadataStructureValidator {
    /**
     ...
     */
    static validateMetadataStructure(path, name, className, properties, schema, context) {
        // Validate the class
        const classPath = path + "/class";
        // The class MUST be defined
        // The class MUST be a string
        if (!BasicValidator_1.BasicValidator.validateString(classPath, "class", className, context)) {
            return false;
        }
        ...
        if (!(0, _3d_tiles_tools_1.defined)(metadataClass)) {
            const message = `The ${name} has a class name '${className}', ` +
            ...
            if (!BasicValidator_1.BasicValidator.validateNumberOfProperties(propertiesPath, "properties", properties, 1, undefined, context)) {
                ...
                for (const propertyName of propertyNames) {
                    ...
                    if (!classPropertyNames.includes(propertyName)) {
                        const message = `The properties of ${name} include a name '${propertyName}', ` +
                            `but the class ${className} does not define this property`;
                        const issue = StructureValidationIssues_1.StructureValidationIssues.IDENTIFIER_NOT_FOUND(path, message);
                        context.addIssue(issue);
                        result = false;
                    }
                }
        ...
        for (const classPropertyName of classPropertyNames) {
            ...
            if (classProperty.required) {
                ...
                if (!(0, _3d_tiles_tools_1.defined)(propertyValue)) {
                    if (classProperty.required) {
                        const issue = MetadataValidationIssues_1.MetadataValidationIssues.METADATA_VALUE_REQUIRED_BUT_MISSING(path, classPropertyName);
                        context.addIssue(issue);
                        result = false;
                    }
                }
...
exports.MetadataStructureValidator = MetadataStructureValidator;
