...
exports.BinaryPropertyTableValidator = void 0;
...
const BinaryPropertyTableValuesValidator_1 = require("./BinaryPropertyTableValuesValidator");
/**
 * A class for validations related to BinaryPropertyTable objects.
 *
 * @internal
 */
class BinaryPropertyTableValidator {
    /**
     ...
     */
    static validateBinaryPropertyTable(path, binaryPropertyTable, context) {
        let result = true;
        ...
        for (const propertyId of Object.keys(classProperties)) {
            ...
            if (!BinaryPropertyTableValidator.validateBinaryPropertyTableProperty(propertyPath, propertyId, classProperty, binaryPropertyTable, context)) {
                result = false;
            }
        }
        ...
        if (result) {
            if (!BinaryPropertyTableValuesValidator_1.BinaryPropertyTableValuesValidator.validateBinaryPropertyTableValues(path, binaryPropertyTable, context)) {
                result = true;
            }
        }
        return result;
     ...
     */
    static validateBinaryPropertyTableProperty(path, propertyId, classProperty, binaryPropertyTable, context) {
        let result = true;
        ...
        const isVariableLengthArray = isArray && !(0, _3d_tiles_tools_1.defined)(count);
        // If the property is a variable-length array, validate
        // the 'arrayOffsets' buffer view.
        if (isVariableLengthArray) {
            if (!BinaryPropertyTableValidator.validateArrayOffsetsBufferView(path, propertyId, binaryPropertyTable, context)) {
                result = false;
        ...
        if (type === "STRING") {
            ...
            if (isVariableLengthArray) {
                numStrings = BinaryPropertyTableValidator.getValidatedArrayOffset(propertyId, propertyTableCount, binaryPropertyTable);
            }
            else if (isArray) {
                numStrings = propertyTableCount * count;
            }
            if (!BinaryPropertyTableValidator.validateStringOffsetsBufferView(path, propertyId, binaryPropertyTable, numStrings, context)) {
                result = false;
        ...
        if (!BinaryPropertyTableValidator.validateValuesBufferView(path, propertyId, classProperty, binaryPropertyTable, context)) {
            result = false;
        }
        return result;
    }
     ...
     */
    static validateValuesBufferView(path, propertyId, classProperty, binaryPropertyTable, context) {
        let result = true;
        const type = classProperty.type;
        ...
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableProperties = (0, _3d_tiles_tools_2.defaultValue)(propertyTable.properties, {});
        ...
        const requiresAlignment = componentType != "UINT8" && componentType != "INT8";
        if (requiresAlignment) {
            // Validate the `values` `byteOffset`
            if (!BinaryPropertyTableValidator.validateBufferViewByteOffset(path, propertyId, "values", valuesBufferViewIndex, componentType, binaryPropertyTable, context)) {
                result = false;
            }
        }
        ...
        const numValues = BinaryPropertyTableValidator.computeNumberOfValues(propertyId, binaryPropertyTable);
        const componentTypeByteSize = _3d_tiles_tools_4.MetadataComponentTypes.byteSizeForComponentType(componentType);
        ...
        if (!BinaryPropertyTableValidator.validateValuesBufferViewByteLength(path, propertyId, valuesBufferViewIndex, componentType, binaryPropertyTable, numValues, expectedByteLength, context)) {
            result = false;
        }
        return result;
    }
     ...
     */
    static validateArrayOffsetsBufferView(path, propertyId, binaryPropertyTable, context) {
        let result = true;
        // Obtain the `PropertyTableProperty`
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableProperties = (0, _3d_tiles_tools_2.defaultValue)(propertyTable.properties, {});
        ...
        const arrayOffsetType = (0, _3d_tiles_tools_2.defaultValue)(propertyTableProperty.arrayOffsetType, "UINT32");
        // Validate the `arrayOffsets` `byteOffset`
        if (!BinaryPropertyTableValidator.validateBufferViewByteOffset(path, propertyId, "arrayOffsets", arrayOffsetsBufferViewIndex, arrayOffsetType, binaryPropertyTable, context)) {
            result = false;
        }
        // Validate the `arrayOffsets` `byteLength`
        const propertyTableCount = propertyTable.count;
        const numArrayOffsetValues = propertyTableCount + 1;
        if (!BinaryPropertyTableValidator.validateOffsetBufferViewByteLength(path, propertyId, "arrayOffsets", arrayOffsetsBufferViewIndex, arrayOffsetType, binaryPropertyTable, numArrayOffsetValues, context)) {
            result = false;
        }
        // Only if the basic properties have been valid,
        // Validate that the offsets are ascending
        if (result) {
            if (!BinaryPropertyTableValidator.validateOffsets(path, propertyId, "arrayOffsets", arrayOffsetsBufferViewIndex, arrayOffsetType, binaryPropertyTable, context)) {
                result = false;
            }
        }
        return result;
     ...
     * @returns Whether the property was valid
     */
    static validateStringOffsetsBufferView(path, propertyId, binaryPropertyTable, numStrings, context) {
        let result = true;
        // Obtain the `PropertyTableProperty`
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableProperties = (0, _3d_tiles_tools_2.defaultValue)(propertyTable.properties, {});
        ...
        const stringOffsetType = (0, _3d_tiles_tools_2.defaultValue)(propertyTableProperty.stringOffsetType, "UINT32");
        // Validate the `stringOffsets` `byteOffset`
        if (!BinaryPropertyTableValidator.validateBufferViewByteOffset(path, propertyId, "stringOffsets", stringOffsetsBufferViewIndex, stringOffsetType, binaryPropertyTable, context)) {
            result = false;
        }
        // Validate the `stringOffsets` `byteLength`
        const numStringOffsetValues = numStrings + 1;
        if (!BinaryPropertyTableValidator.validateOffsetBufferViewByteLength(path, propertyId, "stringOffsets", stringOffsetsBufferViewIndex, stringOffsetType, binaryPropertyTable, numStringOffsetValues, context)) {
            result = false;
        }
        // Only if the basic properties have been valid,
        // Validate that the offsets are ascending
        if (result) {
            if (!BinaryPropertyTableValidator.validateOffsets(path, propertyId, "stringOffsets", stringOffsetsBufferViewIndex, stringOffsetType, binaryPropertyTable, context)) {
                result = false;
            }
        }
        return result;
     ...
     */
    static validateOffsets(propertyPath, propertyId, bufferViewName, bufferViewIndex, componentType, binaryPropertyTable, context) {
        const path = propertyPath + "/" + bufferViewName;
        let result = true;
        const binaryBufferData = binaryPropertyTable.binaryBufferData;
        const bufferViewsData = (0, _3d_tiles_tools_2.defaultValue)(binaryBufferData.bufferViewsData, []);
        const bufferVieData = bufferViewsData[bufferViewIndex];
        const array = _3d_tiles_tools_3.NumericBuffers.getNumericBufferAsArray(bufferVieData, componentType);
        for (let i = 0; i < array.length - 1; i++) {
            const value0 = array[i];
            const value1 = array[i + 1];
            if (value0 > value1) {
                const message = `The '${bufferViewName}' buffer view of property '${propertyId}' ` +
                ...
                const issue = MetadataValidationIssues_1.MetadataValidationIssues.METADATA_INVALID_OFFSETS(path, message);
     ...
     */
    static validateBufferViewByteOffset(propertyPath, propertyId, bufferViewName, bufferViewIndex, componentType, binaryPropertyTable, context) {
        const path = propertyPath + "/" + bufferViewName;
        let result = true;
        ...
        if (misalignment !== 0) {
            ...
            const issue = MetadataValidationIssues_1.MetadataValidationIssues.METADATA_INVALID_ALIGNMENT(path, message);
            context.addIssue(issue);
            result = false;
        }
        return result;
     ...
     */
    static validateOffsetBufferViewByteLength(propertyPath, propertyId, bufferViewName, bufferViewIndex, componentType, binaryPropertyTable, numValues, context) {
        const path = propertyPath + "/" + bufferViewName;
        let result = true;
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableCount = propertyTable.count;
        ...
        if (actualByteLength !== expectedByteLength) {
            ...
            const issue = MetadataValidationIssues_1.MetadataValidationIssues.METADATA_INVALID_LENGTH(path, message);
            context.addIssue(issue);
            result = false;
        }
        return result;
     ...
     * @returns Whether the byte length was valid
     */
    static validateValuesBufferViewByteLength(propertyPath, propertyId, bufferViewIndex, componentType, binaryPropertyTable, numValues, expectedByteLength, context) {
        ...
        const actualByteLength = bufferView.byteLength;
        if (actualByteLength !== expectedByteLength) {
            const message = BinaryPropertyTableValidator.createByteLengthMessage(propertyId, binaryPropertyTable, componentType, numValues, expectedByteLength);
            const issue = MetadataValidationIssues_1.MetadataValidationIssues.METADATA_INVALID_LENGTH(path, message);
            context.addIssue(issue);
            result = false;
        }
     ...
     */
    static createByteLengthMessage(propertyId, binaryPropertyTable, componentType, numValues, expectedByteLength) {
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableCount = propertyTable.count;
        const metadataClass = binaryPropertyTable.metadataClass;
        const classProperties = (0, _3d_tiles_tools_2.defaultValue)(metadataClass.properties, {});
        ...
        const isArray = classProperty.array === true;
        const isVariableLengthArray = isArray && !(0, _3d_tiles_tools_1.defined)(count);
        const componentCount = _3d_tiles_tools_5.MetadataTypes.componentCountForType(type);
        const componentTypeByteSize = _3d_tiles_tools_4.MetadataComponentTypes.byteSizeForComponentType(componentType);
        ...
        if (isVariableLengthArray) {
            ...
            // last string offset)
            if (type === "STRING") {
                const message = `The 'values' buffer view of property '${propertyId}' ` +
     ...
     */
    static computeNumberOfValues(propertyId, binaryPropertyTable) {
        const metadataClass = binaryPropertyTable.metadataClass;
        const classProperties = (0, _3d_tiles_tools_2.defaultValue)(metadataClass.properties, {});
        const classProperty = classProperties[propertyId];
        const propertyTable = binaryPropertyTable.propertyTable;
        ...
        if (isVariableLengthArray) {
            ...
            if (type === "STRING") {
                const lastArrayOffset = BinaryPropertyTableValidator.getValidatedArrayOffset(propertyId, propertyTableCount, binaryPropertyTable);
                const lastStringOffset = BinaryPropertyTableValidator.getValidatedStringOffset(propertyId, lastArrayOffset, binaryPropertyTable);
        ...
        if (isArray) {
            ...
            if (type === "STRING") {
                const lastStringOffset = BinaryPropertyTableValidator.getValidatedStringOffset(propertyId, propertyTableCount * count, binaryPropertyTable);
                return lastStringOffset;
            }
        ...
        if (type === "STRING") {
            const lastStringOffset = BinaryPropertyTableValidator.getValidatedStringOffset(propertyId, propertyTableCount, binaryPropertyTable);
            return lastStringOffset;
        }
     ...
     */
    static getValidatedArrayOffset(propertyId, index, binaryPropertyTable) {
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableProperties = (0, _3d_tiles_tools_2.defaultValue)(propertyTable.properties, {});
        const propertyTableProperty = propertyTableProperties[propertyId];
        const arrayOffsetsBufferViewIndex = propertyTableProperty.arrayOffsets;
        ...
        const arrayOffset = _3d_tiles_tools_3.NumericBuffers.getNumericFromBuffer(arrayOffsetsBufferView, index, arrayOffsetType);
        return arrayOffset;
    }
    /**
     ...
     */
    static getValidatedStringOffset(propertyId, index, binaryPropertyTable) {
        const propertyTable = binaryPropertyTable.propertyTable;
        const propertyTableProperties = (0, _3d_tiles_tools_2.defaultValue)(propertyTable.properties, {});
        const propertyTableProperty = propertyTableProperties[propertyId];
        const stringOffsetsBufferViewIndex = propertyTableProperty.stringOffsets;
        ...
        const binaryBufferData = binaryPropertyTable.binaryBufferData;
        const bufferViewsData = (0, _3d_tiles_tools_2.defaultValue)(binaryBufferData.bufferViewsData, []);
        const stringOffsetsBufferView = bufferViewsData[stringOffsetsBufferViewIndex];
        const stringOffset = _3d_tiles_tools_3.NumericBuffers.getNumericFromBuffer(stringOffsetsBufferView, index, stringOffsetType);
        return stringOffset;
    }
}
exports.BinaryPropertyTableValidator = BinaryPropertyTableValidator;
