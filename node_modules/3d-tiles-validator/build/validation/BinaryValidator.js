...
exports.BinaryValidator = void 0;
...
class BinaryValidator {
     ...
     */
    static validateMinLength(path, name, minLength, actualLength, context) {
        if (actualLength < minLength) {
            const message = `The ${name} must have at least ${minLength} bytes, ` +
     ...
     * @returns Whether the length is valid
     */
    static validateLength(path, name, expectedLength, actualLength, context) {
        if (BigInt(actualLength) !== BigInt(expectedLength)) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_LENGTH(path, name, expectedLength, actualLength);
            context.addIssue(issue);
            return false;
        }
        return true;
     ...
     */
    static validateValue(path, name, expectedValue, actualValue, context) {
        if (actualValue !== expectedValue) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_VALUE(path, name, expectedValue, actualValue);
            context.addIssue(issue);
            return false;
        }
        return true;
     ...
     * @returns Whether the value is valid
     */
    static validateAlignment(path, name, value, expectedAlignment, context) {
        const misalignment = BigInt(value) % BigInt(expectedAlignment);
        if (misalignment > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT(path, name, expectedAlignment, value, misalignment);
            context.addIssue(issue);
            return false;
        }
        return true;
...
exports.BinaryValidator = BinaryValidator;
