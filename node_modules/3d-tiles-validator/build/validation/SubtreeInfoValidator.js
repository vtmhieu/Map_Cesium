...
exports.SubtreeInfoValidator = void 0;
 ...
 * @internal
 */
class SubtreeInfoValidator {
    static async validateSubtreeInfo(path, binarySubtreeData, implicitTiling, context) {
        let optionalSubtreeInfo = undefined;
        try {
            optionalSubtreeInfo = _3d_tiles_tools_3.SubtreeInfos.create(binarySubtreeData, implicitTiling);
        }
        catch (error) {
            if (error instanceof _3d_tiles_tools_5.ImplicitTilingError) {
                const message = `Could not read subtree data: ${error.message}`;
                const issue = SemanticValidationIssues_1.SemanticValidationIssues.IMPLICIT_TILING_ERROR(path, message);
        ...
        if (!SubtreeInfoValidator.validateAvailableCount(tileAvailabilityInfoPath, "tile availability", subtree.tileAvailability.availableCount, tileAvailabilityInfo, context)) {
            result = false;
        }
        // Validate the tileAvailability consistency
        if (!SubtreeInfoValidator.validateTileAvailabilityConsistency(tileAvailabilityInfoPath, tileAvailabilityInfo, implicitTiling, context)) {
            result = false;
        }
        // Validate the childSubtreeAvailability
        const childSubtreeAvailabilityInfo = subtreeInfo.childSubtreeAvailabilityInfo;
        const childSubtreeAvailabilityInfoPath = path + "/childSubtreeAvailability";
        // Validate the childSubtreeAvailability availableCount
        if (!SubtreeInfoValidator.validateAvailableCount(childSubtreeAvailabilityInfoPath, "child subtree availability", subtree.childSubtreeAvailability.availableCount, childSubtreeAvailabilityInfo, context)) {
            result = false;
        }
        ...
        const contentAvailability = (0, _3d_tiles_tools_2.defaultValue)(subtree.contentAvailability, []);
        for (let i = 0; i < contentAvailabilityInfos.length; i++) {
            ...
            const contentAvailabilityInfoPath = contentAvailabilityInfosPath + "/" + i;
            // Validate the contentAvailability availableCount
            if (!SubtreeInfoValidator.validateAvailableCount(contentAvailabilityInfoPath, `content availability ${i}`, contentAvailability[i].availableCount, contentAvailabilityInfo, context)) {
                result = false;
            }
            // Validate that for each available content, there is als
            // an available tile
            if (!SubtreeInfoValidator.validateTileAvailabilityPresence(path, tileAvailabilityInfo, contentAvailabilityInfo, i, implicitTiling, context)) {
                result = false;
            }
        }
        return result;
     ...
     */
    static validateAvailableCount(path, name, availableCount, availabilityInfo, context) {
        ...
        const message = `The ${name} declares an 'availableCount' of ${availableCount} ` +
     ...
     * @returns Whether the availability was consistent
     */
    static validateTileAvailabilityConsistency(path, tileAvailabilityInfo, implicitTiling, context) {
        let result = true;
        const coordinates = _3d_tiles_tools_4.ImplicitTilings.createSubtreeCoordinatesIterator(implicitTiling);
        for (const c of coordinates) {
            const p = c.parent();
            if ((0, _3d_tiles_tools_1.defined)(p)) {
                ...
                if (cAvailable && !pAvailable) {
                    const message = `Tile availability declares tile at ${c} ` +
     ...
     * @returns Whether the availability was consistent
     */
    static validateTileAvailabilityPresence(path, tileAvailabilityInfo, contentAvailabilityInfo, contentAvailabilityIndex, implicitTiling, context) {
        let result = true;
        const coordinates = _3d_tiles_tools_4.ImplicitTilings.createSubtreeCoordinatesIterator(implicitTiling);
        for (const c of coordinates) {
            ...
            if (contentAvailable && !tileAvailable) {
                const message = `Content availability ${contentAvailabilityIndex} declares ` +
...
exports.SubtreeInfoValidator = SubtreeInfoValidator;
