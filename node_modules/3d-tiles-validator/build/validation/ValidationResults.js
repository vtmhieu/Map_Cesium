...
const ValidationIssue_1 = require("./ValidationIssue");
/**
...
class ValidationResults {
     ...
     */
    static filter(input, inclusionPredicate, output) {
        const inputIssues = input.issues;
        const outputIssues = ValidationResults.filterIssues(inputIssues, [], inclusionPredicate);
     ...
     */
    static filterIssues(inputIssues, issuesStack, inclusionPredicate) {
        const outputIssues = [];
        for (let i = 0; i < inputIssues.length; i++) {
            ...
            if (included) {
                const outputIssue = ValidationResults.filterCauses(inputIssue, issuesStack, inclusionPredicate);
     ...
     */
    static filterCauses(inputIssue, issuesStack, inclusionPredicate) {
        const outputIssue = new ValidationIssue_1.ValidationIssue(inputIssue.type, inputIssue.path, inputIssue.message, inputIssue.severity);
        const inputCauses = inputIssue.causes;
        const outputCauses = ValidationResults.filterIssues(inputCauses, issuesStack, inclusionPredicate);
     ...
     */
    static createSimpleResultString(inputResult) {
        let result = "";
        const inputIssues = inputResult.issues;
        for (const inputIssue of inputIssues) {
            const issueString = ValidationResults.createSimpleIssueString(inputIssue, "");
            result += issueString;
        }
        return result;
    }
     ...
     * @returns The string representation
     */
    static createSimpleIssueString(inputIssue, indentation) {
        let result = "";
        result += indentation + inputIssue.type + "\n";
        for (const cause of inputIssue.causes) {
            const causeString = ValidationResults.createSimpleIssueString(cause, indentation + "    ");
            result += causeString;
        }
        return result;
    }
