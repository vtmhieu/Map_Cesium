...
exports.TileFormatValidator = void 0;
...
class TileFormatValidator {
     ...
     */
    static validateHeader(path, input, headerByteLength, expectedMagic, context) {
        if (input.length < headerByteLength) {
            const message = `The input must have at least ${headerByteLength} bytes, ` +
                `but only has ${input.length} bytes`;
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID(path, message);
        ...
        if (magic !== expectedMagic) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_VALUE(path, "magic", expectedMagic, magic);
            context.addIssue(issue);
            return false;
        }
        if (version !== 1) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_VALUE(path, "version", 1, version);
            context.addIssue(issue);
            return false;
        }
        if (byteLength !== input.length) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_LENGTH(path, "content", byteLength, input.length);
            context.addIssue(issue);
            return false;
        }
        return true;
     ...
     */
    static extractBinaryTableData(path, input, headerByteLength, hasEmbeddedGlb, context) {
        const byteLength = input.readUInt32LE(8);
        ...
        if (featureTableBinaryByteOffset % 8 > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT_legacy(path, "feature table binary", 8);
            context.addIssue(issue);
            return undefined;
        }
        if (batchTableJsonByteOffset % 8 > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT_legacy(path, "batch table JSON", 8);
            context.addIssue(issue);
            return undefined;
        }
        if (batchTableBinaryByteOffset % 8 > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT_legacy(path, "batch table binary", 8);
            context.addIssue(issue);
            return undefined;
        }
        if (hasEmbeddedGlb && glbByteOffset % 8 > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT_legacy(path, "GLB data", 8);
            context.addIssue(issue);
            return undefined;
        }
        if (byteLength % 8 > 0) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_ALIGNMENT_legacy(path, "byte length", 8);
            context.addIssue(issue);
            return undefined;
        }
        const computedByteLength = headerByteLength +
        ...
        if (computedByteLength > byteLength) {
            const issue = BinaryValidationIssues_1.BinaryValidationIssues.BINARY_INVALID_LENGTH(path, "header, feature table, batch table, and GLB", byteLength, computedByteLength);
            context.addIssue(issue);
            return undefined;
        }
        const featureTableJsonBuffer = input.subarray(featureTableJsonByteOffset, featureTableBinaryByteOffset);
        const featureTableBinary = input.subarray(featureTableBinaryByteOffset, batchTableJsonByteOffset);
        const batchTableJsonBuffer = input.subarray(batchTableJsonByteOffset, batchTableBinaryByteOffset);
        const batchTableBinary = input.subarray(batchTableBinaryByteOffset, glbByteOffset);
        const glbData = input.subarray(glbByteOffset, glbByteOffset + glbByteLength);
        let featureTableJson;
        let batchTableJson;
        try {
            featureTableJson = _3d_tiles_tools_1.Buffers.getJson(featureTableJsonBuffer);
        }
        catch (error) {
        ...
        try {
            batchTableJson = _3d_tiles_tools_1.Buffers.getJson(batchTableJsonBuffer);
        }
        catch (error) {
...
exports.TileFormatValidator = TileFormatValidator;
