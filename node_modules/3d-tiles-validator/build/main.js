...
const ValidatorMain_1 = require("./ValidatorMain");
const args = yargs(process.argv.slice(1))
    ...
    tilesetsDirectory: {
        type: "string",
        alias: "T",
        describe: "The tileset input directory. This will validate all files " +
    ...
    reportFile: {
        type: "string",
        alias: "r",
        describe: "The name of the file where the report of a single " +
            "validated input file should be written",
    },
    writeReports: {
        type: "boolean",
        alias: "w",
        describe: "Write one report file for each validated file. The file name " +
 ...
 */
async function readOptionsFile(optionsFile) {
    const validationOptions = await (0, readJsonUnchecked_1.readJsonUnchecked)(optionsFile);
    if (!validationOptions) {
        return new ValidationOptions_1.ValidationOptions();
    }
    return validationOptions;
}
async function main() {
    const config = {
        options: new ValidationOptions_1.ValidationOptions(),
    };
    if (argv.configFile) {
        const configFileData = await (0, readJsonUnchecked_1.readJsonUnchecked)(argv.configFile);
        if (!configFileData) {
            return;
        }
    ...
    ValidatorMain_1.ValidatorMain.performValidation(args, config);
}
main();
